...
#define LEVEL3_HUMAN_COUNT 2
#define LEVEL3_ROCK_COUNT 3
...
void Level3::Initialize() {
    ...
    //////////////////////////////////////////////////////////////////////////////////////
    // initialize humans
    state.humans = new Entity[LEVEL3_HUMAN_COUNT];
...
void Level3::Update(float deltaTime) {
    ...
    collisionObj = state.player->Update(deltaTime, state.player, state.humans, LEVEL3_HUMAN_COUNT, state.map);

    for (int i = 0; i < LEVEL3_HUMAN_COUNT; i++) {
        state.humans[i].Update(deltaTime, state.player, state.humans, LEVEL3_HUMAN_COUNT, state.map);
    }
...
void Level3::Render(ShaderProgram* program) {
    ...
    if (state.player->position.x > 5) {
        ...
        Util::DrawText(program, fontTextureID, "Found: " + std::to_string(getHumansFound()) + "/" + std::to_string(LEVEL3_HUMAN_COUNT) + " Humans", 0.5f, -0.25f, glm::vec3(state.player->position.x - 2.0f, -0.5f, 0));
        ...
        Util::DrawText(program, fontTextureID, "Found: " + std::to_string(getHumansFound()) + "/" + std::to_string(LEVEL3_HUMAN_COUNT) + " Humans", 0.5f, -0.25f, glm::vec3(3.0f, -0.5f, 0));
